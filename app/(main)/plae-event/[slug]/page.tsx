...
const GAMMA_BASE = process.env.NEXT_PUBLIC_GAMMA_API_URL!;

interface PlaeEventPageProps {
  params: Promise<{ slug: string }>;
}

export async function generateMetadata({
  params,
}: PlaeEventPageProps): Promise<Metadata> {
  const { slug } = await params;
  const event = await fetchEventBySlug(slug, GAMMA_BASE);
  return {
    title: event ? `${event.title} | DPM` : "Market | DPM",
  };
}

export default async function PlaeEventPage({ params }: PlaeEventPageProps) {
  const { slug } = await params;
  const event = await fetchEventBySlug(slug, GAMMA_BASE);
